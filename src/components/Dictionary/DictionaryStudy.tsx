...
import { Search, Loader2, Book, Volume2, Star } from 'lucide-react';
...
export function DictionaryStudy() {
    ...
    return (
        <div className="flex flex-col h-full max-w-3xl mx-auto px-4 py-6">
            ...
            {/* Added pr-6 to give enough space for scrollbar and content */}
            <div className="flex-1 overflow-y-auto min-h-0 space-y-4 pb-20 pr-6 custom-scrollbar">
                {error && (
                ...
                {results.map((item, idx) => {
                    ...
                    return (
                        <div key={idx} className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-shadow relative">
                            {/* Save Button - High z-index to ensure visibility */}
                            <button
                                onClick={() => handleSave(item)}
                                className={cn(
                                    "absolute top-6 right-6 p-2 rounded-full transition-all border z-10 bg-white dark:bg-gray-800",
                                    bookmarked
                                        ? "bg-amber-100 border-amber-200 text-amber-500 dark:bg-amber-900/20 dark:border-amber-800"
                                        : "border-gray-100 dark:border-gray-800 text-gray-300 dark:text-gray-600 hover:border-amber-200 dark:hover:border-amber-800 hover:text-amber-500 dark:hover:text-amber-400"
                                )}
                            ...
                            <div className="flex items-start justify-between mb-4 pr-16 relative z-0"> {/* Sufficient padding-right for button */}
                                <div>
